...
#' Get a dataframe of metadata from a filename
#'
#' All abf file I generate contain relevant metadata separated by spaces.
#' This function takes a list of filenames and reads the metadata into a dataframe.
#' @param filelist A list of files generated by list.files() must contain the fields
#' date, mouse strain, age, mouse identifier, tissue-piece identifier, cochlear location of piece, and cell identifier.
#' @export
extract.metadata = function(filelist) {
  df = tibble(filename = filelist)
  df %>%
    rowwise() %>%
    mutate(
      metadata = str_split(.data$filename, pattern = " "),
      date = .data$metadata[1],
      strain = .data$metadata[2],
      age = parse_number(.data$metadata[3]),
      mouseid = parse_number(.data$metadata[4]),
      pieceid = parse_number(.data$metadata[5]),
      location = str_sub(.data$metadata[5], end = 2),
      celliddummy = str_split(.data$metadata[6], pattern = "_"),
      cell = .data$celliddummy[1]
    ) %>%
    select(-.data$celliddummy,-.data$metadata)
}
