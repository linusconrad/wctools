...
process.VCstep_baseline = 
  function(abffile){
    ...
    plot0 = VCdata %>%
      filter(.data$t > tjump, .data$t < 0.25) %>%
    ...
    VCdatabysweep = 
      ...
      summarise(V = wctools::measure.timepoint(var = .data$Vm, tvar = .data$t, tp = 0.1, win = 0.1),
                ...
                Ijump195 = wctools::measure.timepoint(var = .data$Imemb, tvar = .data$t, tp = tjump + 0.195, win = 0.05))
    ...
    # #make the layout
    # (bigplot =
    #     (plot0 + plot1) / (TCfit + IKAparamplot) /(plot6) + patchwork::plot_annotation(title = "VC Slow K currents",
